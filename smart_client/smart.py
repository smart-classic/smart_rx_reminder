...
class SmartClient(OAuthClient):
    ...
    def get_tokens_for_record(self, record_id):
        q = """CONSTRUCT {?s ?p ?o} 
                WHERE {?s ?p ?o
        ...
        rec = RDF.Node(uri_string="http://smartplatforms.org/records/%s"%record_id)
        return self.extract_one_token(m, rec)

    def get_all_tokens(self):
        q = """CONSTRUCT {?s ?p ?o} 
                WHERE {?s ?p ?o
                   filter (?p = <http://smartplatforms.org/smart_token> ||
                           ?p = <http://smartplatforms.org/smart_secret> )
                   }"""%record_id
        ...
        for s in m.find_statements(RDF.Statement(
                None, 
                RDF.Node(uri_string="http://smartplatforms.org/smart_token"),
                None)):
            ret.append(self.extract_one_token(m, s.subject))

    def extract_one_token(self, model, record):
        ...
        return OAuthToken(token=ret['smart_token'], secret=ret['smart_secret'])
    ...
    def delete_token(self, record_id):
        ...
        FILTER (
            ?p = <http://smartplatforms.org/smart_token> || 
            ?p = <http://smartplatforms.org/smart_secret>)
        }""" % (record_id, record_id)
    ...
    def save_token(self, record_id, access_token, label=None):
        self.delete_token(record_id)


        sp = RDF.NS("http://smartplatforms.org/")
        rec = sp["records/%s"%record_id]
